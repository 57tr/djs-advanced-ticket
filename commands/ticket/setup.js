...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const row = new ActionRowBuilder().addComponents(
            [
                new ButtonBuilder()
                    ...
                    .setEmoji("<:redticket:1083151929301012491>")
        ...
        setupSchema.findOne({ guildId: guild.id }).then(async (data) => {
            if (data) {
                if (data.channelId === channel.id && data.categoryId === category.id) return interaction.reply({
                    embeds: [returnErrorEmbed("You chose the same category and channel, try others!")],
                    ephemeral: true
                });
